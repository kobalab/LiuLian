...
class LiuLian {
    ...
    noteref(note) {
        ...
        return this._.note.length;
    }

    parse() {
        return this.block() + this.footnote();
    }
    ...
    block(eod) {
        ...
        while (! this.eod) {
            ...
            else if (line.match(/^>>$/))        html += this.blockquote();
            else if (line.match(/^\/[\/\*]/))   html += this.comment();
            else if (line.match(module_re))     html += this.blockModule();
            else                                html += this.p();
        }
        if (eod) this.reset(eod);
        return html;
    }
    ...
    table() {
        let cell = [];
        for (;;) {
            let line = this.readline().replace(/^\|/,'').replace(/\|$/,'');
    ...
    footnote() {
        ...
        for (let i = 0; i < this._.note.length; i++) {
            html += `<li><a id="l-footnote.${i+1}" href="#l-noteref.${i+1}">`
                  +  '^</a> ' + this.inline(this._.note[i]) + '</li>\n';
        }
        html += '</ol>\n</div>\n\n';
        return html;
    }
    ...
    blockModule() {
        ...
        let [ , name, param, eod ]
                = line.match(/^#(\w+)(?:\((.*?)\))?(?:<<(\S+))?$/);
        if (name == '_') return this.tag(line, param, eod);
...
module.exports = function(r) {
    ...
    parser.inline = inline(null,
                           note=>parser.noteref(note));
    return parser.parse();
}
