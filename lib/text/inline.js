...
module.exports = function(parser) {
    ...
    function fixpath(path) {
        if (parser) return parser._r._req.fixpath(path);
        else        return path;
    }

    function bracket(str) {
        if (str.match(/^[^|]*$/)) {
            return `<a href="${cdata(encodeURI(fixpath(str)))}">`
                        + `${cdata(str)}</a>`;
        }
        else {
            ...
            href = encodeURI(
                        decodeURI(
                            fixpath(href).replace(/%(?![0-9a-f]{2})/i,'%25')));
            return `<a href="${cdata(href)}">${inline(title)}</a>`;
        }
    }
