...
function fixpath(req, path = '') {
    if (! path.match(/%[0-9a-f]{2}/i)) path = encodeURI(path);
    return req.fixpath(path);
}
...
module.exports = class Core {
    ...
    icon(type, param) {
        this._r.icon(fixpath(this._req, param));
        return '';
    }
    ...
    script(type, param, value) {
        this._r.script({ url: fixpath(this._req, param), code: value });
        return '';
    }
}
